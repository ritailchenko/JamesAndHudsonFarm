...
if (isset($config))
{
	...
	$env_config['upload_preferences'] = array(
		6 => array(
			'name'        => 'Activities',
			'server_path' => $images_path . '/images/activities/',
			'url'         => $images_url  . '/images/activities/'
		),
	...
	$env_config['show_profiler']        = ( ! ENV_DEBUG OR (isset($_GET['D']) && $_GET['D'] == 'cp')) ? 'y' : 'y' ;
	// Show template debugging if we're not in production
	$env_config['template_debugging']   = (ENV_DEBUG) ? 'y' : 'y' ;
	/**
